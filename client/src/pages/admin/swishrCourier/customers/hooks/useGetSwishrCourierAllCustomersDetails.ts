...
function fetchCustomers(
  ...
  return ApiHelpers.GET(
    ApiConstants.GET_ALL_SWISHR_COURIER_CUSTOMER_DETAILS(),
    {
      params: finalQueryParams,
    }
  );
}
export const useGetSwishrCourierAllCustomersDetails = (
  props: AppGridGetDataAPIProps
) => {
  const { columnFilters, EnableQuery, sorting, pagination } = props;
  const queryKey = [
    'Swishr-Courier-Customers-Grid-data',
    columnFilters,
    sorting,
    pagination,
  ];
